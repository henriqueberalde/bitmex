...
namespace BitMEX
{
    ...
    public class BitMEXApi
    {
        ...
        {
            //2018-02-08T04:30:37Z is 1518064237
            var dataExample = Convert.ToDateTime("2018-02-08T04:30:37");
            var str = dataExample.ToUniversalTime();
            var str2 = dataExample.Subtract(new DateTime(1970, 1, 1)).TotalSeconds;
            return (long)DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1)).TotalSeconds + 3600;
            //return DateTimeOffset.UtcNow.ToUnixTimeSeconds() + 3600; // set expires one hour in the future
        }

        private T Query<T>(string method, string function, Dictionary<string, string> param = null, bool auth = false, bool json = false)
        {
            var result = Query(method, function, param, auth, json);
            ...
            {
                throw new Exception($"Erro ao converter a string: {result}", ex);
            }
        }
        ...
        public List<WalletHistoryItem> GetUserWalletHistory(bool reversed)
        {
            ...
            param["count"] = 100.ToString();
            param["start"] = 0.ToString();
            param["reverse"] = reversed.ToString();
            //param["startTime"] = "";
            //param["endTime"] = "";
            return Query<List<WalletHistoryItem>>("GET", "/user/walletHistory", param, true, false);
        }
